...
// @@@@@ ：GSPが「CascadeType.ALL」属性の付与に対応していないので個別に取得 @@@@@
		...
		od1.setOrderId(order.getOrderId()); // @CascadeType.*
		od1.setTestOrder(order);
		od1.setProductId(product1.getProductId()); // @CascadeType.*
		...
		od2.setOrderId(order.getOrderId()); // @CascadeType.*
		od2.setTestOrder(order);
		od2.setProductId(product2.getProductId()); // @CascadeType.*
		...
		List<TestOrder> ordersOfQuery = entityManager.createQuery("from TestOrder").getResultList();
